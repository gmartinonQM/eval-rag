...
from audit_llm.faithfulness.dataclass import (
    LLMJudgeInput,
)
from audit_llm.faithfulness.utils_faithfulness import (
    rolling_concatenate_string_dynamic,
)
...
def preprocess_input_data(source_data: pd.DataFrame) -> pd.DataFrame:
    """
    Preprocesses the source data by adding a column that contains the rolling
    concatenation of sentences within each paragraph.

    Parameters
    ----------
    source_data : pd.DataFrame
        A DataFrame containing the source data with columns such as
        "sentence" and "paragraphe".

    Returns
    -------
    pd.DataFrame
        A DataFrame with an additional column "previous_sentences", where each
        row contains the concatenated sentences up to and including the current
        sentence within the same paragraph.
    """
    source_data["previous_sentences"] = (
        source_data.groupby("paragraphe")
        .apply(rolling_concatenate_string_dynamic)
        .reset_index(level=0, drop=True)
    )

    source_data["source_sentence_content_str"] = source_data[
...
def run_llm_as_judge(
    ...
    for batch_index in tqdm(
        ...
        print("\nbatch index : start_index, end_index", start_index, end_index)
        ...
        input = [
            LLMJudgeInput(
                sources=data["source_sentence_content_str"],
                previous_sentences=data["previous_sentences"],
                phrase=data["sentence"],
            ).model_dump()
...
def compute_metric_score(
    ...
    output_llm_judge : pd.DataFrame
        ...
        'sources_sentence_content', 'sentence', and 'predict_llm_judge_metric'
        (dict with 'raison' and 'label').

    Returns
    -------
    results : dict of str, Any
        Dictionary mapping row indices to extracted sources_sentence_content,
        sentence, reason, and binary label.
    ...
    for index, row in output_llm_judge.iterrows():
        try:
            results[index] = {
                "sources_sentence_content": row["sources_paragraphe_content"],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metric",
        required=True,
        help="The metric to be used for processing, e.g., 'faithfulness'",
        choices=[
            "faithfulness",
            "speculation",
        ],
    ...
    root_abspath = "../../data/10_output_pipeline/12-faithfulness"
    file_name = os.getenv("DATA_SOURCE_NAME_FAITHFULNESS")
    ...
    if is_tmp_empty:
        ...
        input_data_preprocessed = pd.read_pickle(f"tmp/tmp_{metric}.pkl")
